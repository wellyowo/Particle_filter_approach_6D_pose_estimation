...
if __name__ == '__main__':
    ...
    if opt.dataset == "ycb":
        for now in tqdm(range(opt.nth_photo, opt.nth_photo+1)):
            ...
            # delete itemid of labels which are not in rois (photo)
            ...
            #generate objects_region_mask
                
                
            for itemid in labels:
                if itemid not in posecnn_meta['rois'][:,1] or itemid not in indices:
                    print("Error : ", itemid, " is not in rois or indices")
                    continue
                ...
                print("itemid : ", itemid, " best_score : ", best_score, " pose : ", pose)
                if best_score != 0:
